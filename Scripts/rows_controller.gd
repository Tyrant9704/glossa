...
@onready var keyboard
@onready var validation_alert = %ValidationAlert
@onready var results_ui = %ResultsUI

@onready var key_standard = %Keyboard_standard
@onready var key_pol = %Keyboard_pol
@onready var key_ukr = %Keyboard_ukr
...
func _ready():
	...
	match global.words_lang:
		'eng': keyboard = key_standard
		'pol': keyboard = key_pol
		'ukr': keyboard = key_ukr
	
	keyboard_choose()
	

func keyboard_choose():
	var all_keyboards = get_tree().get_nodes_in_group('keyboard')
	
	for i in all_keyboards:
		if i == keyboard:
			i.set_visible(true)
